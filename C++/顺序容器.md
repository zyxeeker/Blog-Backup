...
|`s.erase(pos, len)`|删除从位置 *pos* 开始的 *len* 个字符。省略 *len* 则删除到尾|
|`s.assign(args)`|将 *s* 中的字符替换为 *args* 指定的字符|
|`s.append(args)`|追加 *args* 到 *s* 后面|
|`s.replace(range, args)`|删除 *s* 中方位 *range* 内的字符，替换为 *args* 指定的字符|
...
|*n ,c*|*n* 个字符 *c*|
|*b, e*|迭代器 *b* 和 *e* 范围内的字符|

除上述之外还有一个初始化列表（`{...}`）

### string 搜索操作
>`string` 搜索函数返回的是 `string::size_t` 类型
...
|`s.find_first_no_of(args)`|在 *s* 中查找第一个不在 *args* 中的字符|
...
|参数|说明|
|:----|:----|
|*c, pos*|从 *s* 中位置 *pos* 处开始查找字符 *c*。*pos* 默认为 0|
|*s2, pos*| 从 *s* 中位置 *pos* 处开始查找**字符串** *s2*。*pos* 默认为 0|
|*cp, pos*|从 *s* 中位置 *pos* 处开始查找指针 *cp* 指向的C风格字符串。*pos* 默认为 0|
|*cp, pos, n*|从 *s* 中位置 *pos* 处开始查找指针 *cp* 指向的C风格字符串的前 *n* 个字符。*pos* 与 *n* 无默认值|
...
|`s.compare(pos1, n1, n2)`|将 *s* 中从 *pos1* 开始的 *n* 个字符与 *s2* 进行比较|
|`s.compare(pos1, n1, s2, pos2, n2)`|将 *s* 中从 *pos1* 开始的 *n1* 个字符与 *s2* 中从 *pos2* 开始的 *n2* 字符进行比较|
|`s.compare(cp)`|比较 *s* 与 *cp* 指向的以空字符为结尾的字符数组进行比较 |
|`s.compare(pos1, n1, cp)`|将 *s* 中从 *pos1* 开始的 *n1* 个字符与 *cp* 指向的以空字符为结尾的字符数组进行比较|
|`s.compare(pos1, n1, cp, n2)`|将 *s* 中从 *pos1* 开始的 *n1* 个字符与 *cp* 指向的地址开始的 *n2* 个字符进行比较|

### 数值转换
>如果 `string` 不能转换为一个数值，将会抛出 `invalid argument` 异常；若转换得到的数值无法用任何类型进行表示，则抛出 `out_of_range` 异常

`to_string(val)` 是一组重载函数，*val* 可以是任何算术类型，每种算术类型都有相应的重载函数

`stoi(s, p, b)`，`stol(s, p, b)`，`stoul(s, p, b)`，`stoll(s, p, b)`，`stoull(s, p, b)`：返回 *s* 的起始子串的（表示整数内容）数值，返回类型为整形。*b* 表示转换所用的基数，默认值为 10。 *p* 是`size_t` 指针，用来保存 *s* 中第一个非数值字符的下标，*p* 默认为 0，代表函数不保存下标

`stof(s, p)`，`stod(s, p)`，`stold(s, p)`：返回 *s* 的起始子串的（表示浮点数内容）数值，返回类型为浮点型。*p* 作用跟上面相同

## 容器适配器
还有三个顺序容器适配器：`stack`，`queue` 和 `priority_queue`。容器适配器可以接受一种已有的容器类型，使其操作更加简便

|操作|说明|
|:----|:----|
|`size_type`|类型，保存当前类型的最大对象的大小|
|`value_type`|元素类型|
|`container_type`|适配器底层的容器类型|
|`A a;`|创建空适配器|
|`A a(c);`|创建一个带有容器 *c* 拷贝的适配器|
|`a.empty()`|一个意思|
|`a.size()`|同理|
|`swap(a, b)`|交换元素|
|`a.swap(b)`|交换元素|
...
默认情况下，`stack` 和 `queue` 是基于 `deque` 实现的；`priority_queue` 则是 `vector` 

- 适配器允许将容器作为一种类型
- 适配器要求容器具有添加和删除元素以及访问尾元素的能力
- `queue` 适配器要求 `back`，`push_back` 和 `push_front`
- `priority_queue` 除了跟 `queue` 要求相同外，还要求随机访问的能力

### stack
前表未列出的操作：
`s.pop()`，`s.push(item)`，`s.emplace(args)`，`s.top()`

### queue

前表未列出的操作：
`q.pop()`，`q.push(item)`，`q.emplace(args)`，`q.top()`，`q.front()`，`q.back()`